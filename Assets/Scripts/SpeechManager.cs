...
public class SpeechManager : MonoBehaviour
{
    ...
    public VoiceName voiceName = VoiceName.ltLTLeonasNeural;
    public int VoicePitch = 0;
    public int completedAudioClips = 0;
    ...
    public void PlayAudioClips()
    {
        if (!isPlaying)
        {
            StartCoroutine(PlayAudioClipsCoroutine());
        }
    }

    IEnumerator PlayAudioClipsCoroutine()
    {
        isPlaying = true;
        while (currentClipIndex < audioClips.Count)
        {
            ...
            audioSource.Play();
            yield return new WaitForSeconds(audioClips[currentClipIndex].length + delayBetweenClips);
            currentClipIndex++;
        }
        isPlaying = false;
    }
    ...
    private void PlayAudio(Stream audioStream)
    {
        ...
        if (size > 0)
        {
            try
            {
                ...
                Debug.Log("Added a clip to audioClips list");
                audioClips.Add(clip);
    ...
    public void SpeakWithSDKPlugin(string message, VoiceName voice, int currentIndex, int dialogueCount)
    {
        Synthesize cortana = new Synthesize();
        SpeechSynthesizer synthesizer;
        Debug.Log($"SpeakWithSDKPlugin called for index {currentIndex}.");
        // Creates an instance of a speech config with specified subscription key and service region.
        var config = SpeechConfig.FromSubscription(SpeechServiceAPIKey, SpeechServiceRegion);
        config.SpeechSynthesisLanguage = cortana.GetVoiceLocale(voice);
        config.SpeechSynthesisVoiceName = cortana.ConvertVoiceNametoString(voice);
        ...
        {
            StartCoroutine(WaitAndPlayRoutineSDK(Speaking, currentIndex, dialogueCount));
        });


    }

    private IEnumerator WaitAndPlayRoutineSDK(Task<SpeechSynthesisResult> speakTask, int currentIndex, int dialogueCount)
    {
        // Yield control back to the main thread as long as the task is still running
        while (!speakTask.IsCompleted)
        {
        ...
        if (result.Reason == ResultReason.SynthesizingAudioCompleted)
        {
            ...
            // Debug.Log($"Converting audio data of size {unityData.Length} to Unity audio clip with {sampleCount} samples at frequency {frequency}.");
            var clip = ToClip("Speech", unityData, sampleCount, frequency);
            //  Debug.Log($"Assigning AudioClip at index {currentIndex}.");
            // Set the source on the audio clip
            audioClips[currentIndex] = clip;
            // Debug.Log($"AudioClip assigned at index {currentIndex}.");
            completedAudioClips++;

            // Check if all audio clips have been generated
            if (completedAudioClips == dialogueCount)
            {
                Debug.Log("All dialogue audio files generated successfully!");
    ...
    public async void SpeechPlayback()
    {
        if (isReady)
        {
            ...
            List<OpenAI.ChatGPT.DialogueLine> dialogues = OpenAI.ChatGPT.instance.dialogue;

            // Initialize the audio clips list to the appropriate size
            audioClips = new List<AudioClip>(dialogues.Count);
            for (int i = 0; i < dialogues.Count; i++)
            {
                audioClips.Add(null);
            }

            // Group the dialogues by speaker name
            var dialoguesBySpeaker = dialogues.GroupBy(d => d.SpeakerName);

            foreach (var speakerGroup in dialoguesBySpeaker)
            {
                VoiceName voiceName;
                switch (speakerGroup.Key)
                {
                    case "Tomai":
                        voiceName = VoiceName.ltLTLeonasNeural;
                        break;
                    case "Laura":
                        voiceName = VoiceName.ltLTOnaNeural;
                        break;
                    default:
                        Debug.LogWarning($"No voice found for speaker {speakerGroup.Key}.");
                        continue;
                }

                foreach (var dialogue in speakerGroup)
                {
                    if (dialogue == null)
                    {
                        Debug.LogWarning($"Dialogue is null for speaker {speakerGroup.Key}.");
                        continue;
                    }

                    string msg = dialogue.SpokenText;

                    VoicePitch = 1;

                    // Add SpeakWithSDKPlugin to tasks
                    Debug.Log($"Calling SpeakWithSDKPlugin for speaker {speakerGroup.Key} and index {dialogue.Index}.");
                    int currentIndex = dialogue.Index;
                    tasks.Add(Task.Run(async () =>
                    {
                        await Task.Delay(currentIndex * 100);
                        SpeakWithSDKPlugin(msg, voiceName, currentIndex, dialogues.Count);
                    }));
                }
            }
